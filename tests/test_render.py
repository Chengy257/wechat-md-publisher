...
class TestRenderArticle:
    ...
    def test_preview_shell_includes_codeblock_dots_and_copy_button(self, tmp_path: Path):
        ...
        assert '<span class="copy-icon copy-icon-back">\u00a0</span>' in result.preview_html
        assert "复制代码" not in result.preview_html
        # Preview shell styles the dots, positions the copy icon like the
        # themes do (right: 8px), and moves the lang label out of the way.
        assert ".codeblock-bar .codeblock-dot" in result.preview_html
        assert ".copy-btn { position: absolute; right: 8px; top: 50%;" in result.preview_html
        assert ".codeblock-bar .codeblock-lang { position: absolute; right: 44px;" in result.preview_html
        # The copy script binds the existing span instead of skipping it.
        assert "bar.querySelector('.copy-btn')" in result.preview_html
        assert "'\\u2713'" in result.preview_html
        assert "'\\u2715'" in result.preview_html
        assert "btn.innerHTML = iconHTML" in result.preview_html
